...
async function lerArquivoBarros(req, res) {
  ...
  async function lerArquivoGabriel(req, res) {
    try {
      const { empresa, dia } = req.params;

      const fileKey = `${empresa}/${dia}/etl.json`; 
...
module.exports = {
  lerArquivo, lerArquivoBarros, lerArquivoGabriel, lerArquivoMatheus
};
